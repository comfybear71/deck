...
- **Next**: **Sunnybank** — an Australian outback adult-cartoon sitcom,
  a sibling to the music-video flow, not a variant of it (no song/MP3
  anywhere in it) — is now `enabled: true` and has its own first real
  screen (`components/SkidmarksSunnyBanksPanel.tsx`, 2026-09-15): a
  cast strip of the six locked series regulars
  (`lib/sunnyBanks.ts`'s `SUNNY_BANKS_CAST`) plus a Script card:
  paste a newline-split block, the panel parses it into a dense
  one-row queue (Act I/II/III swap three in-memory script buffers —
  not a Neon act table), and Render walks that queue one real
  ...
  finished MP4s sit in one Act-grouped Clips shelf after the script
  (same `w-44`/`h-28` `touch-pan-x` cards as the music-video rendered-
  clips strip, not in each dialogue row); the Episode workspace shelf
  snapshots those buffers on the existing Neon session row (not
  `localStorage`, not a new Neon episode/beat table), one card per
  episode name covering every act, each with a red ✕.
  ...
  "Skidmarks node" section getting updated to match; flagging that gap
  explicitly rather than re-asserting the old claim).
...
| Finished-song archive | **Real** — "Archive" snapshots the live band+mp3 (segments, plates, prompts, motion text) to Vercel Blob JSON + carries forward the mp3's own audio URL, lists in a page-bottom shelf **one row per band + MP3 filename** (2026-09-17: a later Archive replaces the older card; leftover copies collapse on the next shelf load, keeping the row with more renders — My Best Friend ×3 / Simulation.mp3 empty pair / Give Me Something 27+22 were the live report), "Open in editor" restores it (auto-archiving whatever's currently live first), "Download project zip" bundles prompts/stills/renders/audio "as practical," **plus a per-song `brief.txt`/`plan.txt`** (Stuart's own director-workflow templates, auto-filled with whatever the app already knows — Song/Band/Length/MP3, and a locked character's `directorNote` as the Artist line — everything else left blank for him to fill by hand; `docs/skidmarks/director-brain.txt` is the one persistent, cross-song counterpart to these two and is deliberately *not* bundled into any song's zip). Deliberately still Blob, not Neon, for this one piece — see `lib/skidmarksArchive.ts`'s own module doc comment for why (this is unrelated to Neon now being real for the session mirror itself, see the Neon section above) | `lib/skidmarksArchive.ts`, `lib/skidmarksArchiveDedupe.ts`, `app/api/skidmarks/archive/route.ts`, `app/api/skidmarks/blob-upload/route.ts`, `components/SkidmarksArchiveShelf.tsx`, `docs/skidmarks/director-brain.txt` |
...
| Sunny Banks (Sunnybank landing tile) | **Real, but deliberately a one-beat pilot, not the episode wizard** (2026-09-15) — `enabled: true` now (was `false`); tapping the tile renders `SkidmarksSunnyBanksPanel`, a **cast strip** of the six locked series regulars (`lib/sunnyBanks.ts`'s `SUNNY_BANKS_CAST` — Shazza, Dazza, Nan, Nuggets, Ranger Bazza, Unit 4S; Hans is a one-episode guest, kept in the same table but never shown in this always-on strip) plus a **Script card**: paste a newline-split block; `parseSunnyBanksScriptBlock` in `components/SkidmarksSunnyBanksPanel.tsx` turns each line into a dense one-row spreadsheet line (detected `SUNNY_BANKS_CAST` name, dialogue fragment, per-row location `<select>` (no act-level location dropdown above the script accordion), Idle/Rendering…/Done pill; a Done row is static — name + spoken-line `<details>` under it, no character/location picks, no 40px black preview cube). God Script headers (`# EPISODE:`, `=== ACT`, `[Location: id]`, `[Action: text]`) set the episode name, split in-memory act buffers, carry a locked park plate into `startImageDataUrl` for following rows, and append action text after the gold prompt. `=== LABEL ===` scene headers (e.g. `=== THE EPISODE TAG ===`, `=== ACT III — CROWD CUTAWAY ===`) stay in the in-memory parse array as sequence chunks and are not spreadsheet/queue rows; blank, tag-only lines skip the queue. Empty non-CAST `Crowd:` is a location Hold cutaway (locked park plate + `[Action:]` as motion, no CAST overlay, gold Hold unused) so it can sit Idle while Act III seed Speaks stay Done — Render then reads "Render 1 line" instead of "Clips already loaded". `[Character Dazza wrapped in bandages]` (no colon) is a look tag on the next speaker, not a row. `[Character Name: look]` strips brackets into per-row `appearanceModifier` (appended after gold via the existing `action` suffix plus its own POST field next to `characterName`; gold in `lib/sunnyBanks.ts` is not rewritten). Queue rows do not scroll horizontally on a 390px phone. Done rows are static (name, spoken line under it, pill on the right); Idle/failed rows keep character + location `<select>`s plus a spoken-line field (empty = Hold). No in-row 40px preview cube. A **+** on each queue row inserts a silent Hold under that clip (same speaker/plate) so a shot can land between two Done rows without re-pasting the God Script; Render then reads "Render 1 line". − on an Idle/failed row drops that accidental shot. Re-parsing the script (`preserveRenderedRuntimes`) keeps an in-memory Done clip when speaker + dialogue still match instead of idling it; **↩ Undo** beside the Act pills restores the previous textarea/act buffers so the queue recompiles. Tag-only `[Character Name: override]` / `[Location:]` / `[Action:]` lines never mint ghost Idle rows. Render POST sends `line` / `action` / `locationId` to the speak-beat route (`action` after gold, never into TTS) — not a Neon schema, not a layout change. Act I/II/III pills sit at the top of the Script card in a horizontal `touch-pan-x` scroll strip, with the script tools (`⇥ Format` / `⤢ Full screen` / `↩ Undo`) on their own row underneath. **Two separate rows is the fix** (2026-09-18): acts and tools used to share one strip, so adding the Full screen button pushed Act I/II/III clean off the left edge and they read as missing. Wrapping the acts fixed that but cost two rows of vertical space on a 390px phone for a one-line control, so they scroll again — just in a strip that only ever holds acts and can't be crowded out by a tool button. `actStripRef` scrolls a freshly added act into view, since in a scroll strip it can land past the right edge. Buttons in this panel are `rounded-md`, not `rounded-full` (2026-09-18, Stuart: "square edge buttons only slightly rounded... them big round buttons, I don't like them") — the colour-key dots and the Idle/Done status chip stay round, being indicators rather than buttons. **+ Add Act** appends the next roman in-memory buffer (IV, V, …, cap 20) with an empty script — not a Neon act table. Save and the episode zip walk `actIds` in order so extra acts are not dropped. The textarea and queued rows collapse behind **Show Script Text & Queued Lines** (default closed) so a 46-line paste doesn't bury the Clips strip. Script card uses `touch-pan-y overscroll-y-contain`; queue rows use `touch-action: pan-x pan-y` so iPhone Safari vertical page scroll isn't trapped. Finished MP4s sit in one default-open **Clips** shelf after the script, same `w-44`/`h-28` `touch-pan-x` cards as `SkidmarksRenderedClipsShelf`, grouped Act I / II / III, inline `<video controls>` (not a lightbox). **The panel opens on Crash Lab EP02 Drop Bears** (`mgen_20260827092841004_ea9` — 46 already-done MP4s streamed from `skidmarks.aiglitch.app`, Act I caravan / Act II BBQ / Act III Unit 9+tag) so the strip can be judged with real clips. In-memory seed only — not a re-render, not a Neon episode table, and Render skips lines that already have a clip. **New Episode (2026-09-18)** — live QA: "how do I create a new episode? I want to create new and then it clears everything, all the old stuff on the workspace should be saved." There was **no such control at all**: the panel could save, download, open and delete an episode but never begin one. `startNewSunnyBanksEpisode` replaces `live` with `buildEmptySunnyBanksLive()` — genuinely blank, deliberately **not** `buildDefaultSunnyBanksLive()`, which is the Crash Lab EP02 demo seed. `workspaces` are untouched, so saved episodes survive. Two taps, because it discards the live copy: the first arms it and says whether what's on screen is saved yet (`isSunnyBanksLiveSaved`, fingerprint match against the saved cards — a rename counts as unsaved, since the saved card really does still carry the old name, and erring that way costs one extra Save tap rather than the work). A blank episode still counts as user content, so a refresh cannot reseed EP02 over it. **Clip proxy (2026-09-18)** — live QA: the zip shipped **18 of 64 clips**. The 18 were this app's own renders on Vercel Blob; the 46 that failed were the Crash Lab EP02 seed clips on `skidmarks.aiglitch.app`. A browser plays a cross-origin MP4 in a `<video>` fine but refuses to let page JavaScript read its bytes without a CORS header, and that host doesn't send one — so `fetch` threw and the clip was silently dropped. `GET /api/skidmarks/sunnybank/clip-proxy?url=…` now streams a clip back same-origin (CORS is a browser policy, never applied server-to-server), and `fetchSunnyBanksBinaryAsset` retries through it only when the direct fetch fails — a Blob URL already works cross-origin and proxying it too would double the bytes. **The allowlist in `lib/sunnyBanksClipProxy.ts` is the security boundary**: a route that fetches whatever a query param names is an open proxy into anything the deployment can reach. HTTPS only, exact host match or a dot-anchored `.public.blob.vercel-storage.com` suffix, no userinfo, no port, non-media content types refused, and the URL is checked **before** any fetch (the fetch itself is the vulnerability). Tested separately from the route, refusals first. **Not live-verified** — this sandbox is blocked from reaching either clip host. **Save / Download / Open, with visible results (2026-09-18)** — live QA: "cannot download episodes", "I don't even know if the save button is working", "how do we open it back up in an editor?". All three already worked and all three were silent or unlabelled. **Save Episode** (renamed from Save Project Workspace) now names what it saved; **Download Episode (.zip)** reports `Getting clip N of M…` per clip (`buildSunnyBanksEpisodeBundle`'s new `onProgress`) and then how many clips actually made it in — `clipCount` vs `fetchedClipCount` are genuinely different when a stream drops, and claiming otherwise would be a lie. Each saved episode is a full-width row with named **Open in editor** and **Download (.zip)** buttons, replacing the sideways-scrolling card whose only affordance was that the whole card happened to be an open target. Download works straight off a saved card without opening it first (`collectEpisodePrompts` takes a source rather than closing over live state), so pulling an old episode never replaces what's currently open. Named **Save Project Workspace** writes the whole live episode (every act, clip URLs, scripts) onto the existing Neon session row — not a new episode/beat table, not `localStorage`. Same episode name updates that one card (`3 acts · 46 clips`, never `Act II` alone). The live working copy persists beside the shelf so a refresh or closed sheet does not reseed EP02. ✕ drops that named card only. **Download Episode Bundle** (`lib/sunnyBanksEpisodeBundle.ts`) fetches each clip's MP4 and matching driving-audio MP3 into `video/NN.mp4` / `audio/NN.mp3` (Crash Lab `voiceFile` for EP02 seed clips; a dropped stream is skipped) and writes `script.txt`, gold prompts, and URL records under `data/`. Render walks that queue via `POST /api/skidmarks/sunnybank/generate-speak-beat` — ElevenLabs text-to-speech (`lib/elevenLabsSpeech.ts`, first caller going TTS-direction rather than Scribe transcription, same `ELEVENLABS_API_KEY`) — a Speak line under LTX's 2s LoadAudio floor (live QA: Shazza "You right?" → 0.8s) gets a silent MP3 tail (`padMp3ToMinimumDurationSec`), not a 422 — then muxes that same driving MP3 onto the finished MP4 (`lib/muxClipAudio.ts`) because LTX SaveVideo does not keep the audio track (live QA: lips moved, no sound) — feeds the same Comfy Cloud LTX 2.3 IA2V graph Skidmarks' own music-video Vocal render already calls (`lib/comfyCloud.ts`, `COMFY_CLOUD_API_KEY`). **Silent Hold (2026-09-17)** is a second one-clip tap on that same form / same route (`kind: "hold"`): skips ElevenLabs, encodes a 5s silent MP3 (`lib/silentMp3.ts`, `SUNNY_BANKS_HOLD_DURATION_SEC`) because LTX's `LoadAudio` node still requires audio, uses `buildSunnyBanksHoldPrompt` (gold, no dialogue, no invented shoes/shadow-silhouette), still a paid Comfy Cloud LTX render — never a parallel fan-out, never a ShotBlock schema. **Appearance changes reach the picture, not just the motion prompt (2026-09-18)** — a real reported bug: telling LTX a character is "holding two bottles" via the motion prompt alone morphed/duplicated the bottles mid-clip, because the xAI-composited starting frame never showed them, so LTX had to invent the prop from nothing. `appearanceModifier` now also feeds `compositeSunnyBanksCharacterOntoLocation`'s xAI edit prompt (`buildSunnyBanksCompositePlatePrompt`'s new optional override param) so the starting frame already shows it, in addition to still being appended to the motion prompt as before. **No settle pause (2026-09-18)** — PR #142 shipped `SUNNY_BANKS_SETTLE_LEAD_SEC` (1.5s) of silence prepended to a Speak beat carrying an `appearanceModifier`, plus a prompt sentence telling the character to hold the newly-staged pose before speaking. Stuart dropped it on sight the same day: with the change baked into the composed plate, frame 0 *is* the new action place, so there is nothing to settle into — the beat goes straight there and starts talking. Duration and driving audio are exactly what ElevenLabs returned; `SUNNY_BANKS_SETTLE_LEAD_SEC` and `prependSilenceToMp3` are gone rather than left unused. `padMp3ToMinimumDurationSec` is a different thing and stays (LTX's hard 2s audio floor, not a deliberate pause). Don't reintroduce a lead-in without him asking. **God Script Cheat Sheet (2026-09-18, Stuart's ask)** — a default-closed 44px row under the script section (`SunnyBanksGodScriptCheatSheet`, content from `lib/sunnyBanksGodScriptGuide.ts`) listing the three real tags, the silent-hold shape, the "a tag is used up by the next row" rule, the speaking cast and the location ids, plus a **Copy these rules as an AI prompt** button (`buildSunnyBanksGodScriptPrompt`) since the scripts are drafted in an LLM chat. Cast/location lists are derived from `SUNNY_BANKS_CAST` / `SUNNY_BANKS_LOCATIONS`, never retyped, and `lib/sunnyBanksGodScriptGuide.test.ts` runs the guide's own worked example through the real `parseSunnyBanksScriptBlock` so a parser change can't leave the cheat sheet teaching a shape that costs paid renders. Prompted by live QA: a drafted Act IV used `[Outfit: ...]` and `[silence]` (neither is a real tag — both would have been read aloud by ElevenLabs in a billed clip) and arrived as one unbroken line. Documentation only — not a validator, not a linter. **Full-screen God Script editor (2026-09-18, Stuart's ask)** — `⤢ Full screen` beside `⇥ Format` opens `SunnyBanksFullScreenScriptEditor`, a fixed-inset sheet whose text is a **local draft**: nothing re-parses, re-derives the queue or autosaves until **Done** runs the same `handleScriptChange` once. The point is the draft buffer, not the size — the inline textarea re-parses on every keystroke, so a half-typed line stops matching its rendered clip, `preserveRenderedRuntimes` can't rebind it and the row flickers Idle mid-edit (nothing is billed by that; Render is still a deliberate tap). Carries the same highlight overlay and its own Format button. **Deliberate trade-off, stated in the sheet's own header**: while it is open the draft is not saved, which is the one screen in this panel that opts out of keystroke autosave; Cancel on a changed draft asks first. A localStorage draft mirror with a recover prompt is the obvious next step, not built speculatively. **Finished shots collapse (2026-09-18, Stuart's ask)** — a default-closed `N finished shots` row above the queue hides `status === "done"` rows; a failed row and the row currently rendering always stay visible (unfinished work and the thing you are watching, not history). The old prose hint under the textarea (one-speaker-per-line, + / −, Unit 4S stays barefoot) is **gone** — all of it now lives in the God Script Cheat Sheet, and two copies of the same rules is how they drift apart; the colour key stays, since that is a legend for what the textarea is doing rather than documentation. **Script textarea text was invisible (2026-09-18)** — the tag-highlight overlay (`SunnyBanksScriptHighlightOverlay`) is the only thing that draws the God Script textarea at all: the real `<textarea>` is `text-transparent` so the colored tags show through it. It shipped with a `text-white/0` base and only tag spans got a color, so every ordinary dialogue line rendered black-on-black on a real iPhone. `SUNNY_BANKS_HIGHLIGHT_CLASSES` now carries a `plain: "text-white"` entry and the overlay colors every segment from that table; a unit test asserts no entry is transparent (`/0`). Keep every value in that table opaque. **Hero still, not the turnaround sheet (2026-09-17)** — live QA: Hold on Shazza animated every cell of `shazza-reference.jpg` because that file is a 4-body + 4-head character plate and the gold prompt keeps the start image's people. Speak/Hold `startImageDataUrl` is the location canvas; the route composites the hero onto it before LTX. The cast strip uses `resolveSunnyBanksStartImage` (`*-hero.jpg` cropped once from the sheet; Unit 4S already a single figure). Not a runtime canvas cropper, not a pose picker. **Location plates (2026-09-17)** — six locked full-frame park stills (`SUNNY_BANKS_LOCATIONS`: water tank dam, main entrance, site laundry, office storefront, tin shed & mower, fibro caravan interior). A native Locations `<select>` under the character row; Speak/Hold POST that still as `startImageDataUrl` (location canvas, Image 1) plus `locationId`/`locationImage` alongside `characterName`. The speak-beat **route** overlays the hero card as Image 2 via xAI edits (copied from Studio `plateCastIntoGen` — not a second LTX LoadImage), then sends the composed still to node `269`. Not a sequencer. Every look/voice/style lock string in `lib/sunnyBanks.ts` is copied verbatim from the original Skidmarks Studio repo's own "worked 100%, do not rebuild these" gold doc — never reworded. Dazza Speak/Hold prompts append `SUNNY_BANKS_HELD_OBJECT_LOCK` after the gold template so beers/coins/hair dryer cannot morph mid-clip; Shazza's field-for-field gold `toBe` is unchanged. **Cast thumbnails are square tiles in one horizontally-scrollable strip** (2026-09-17, matches `SkidmarksBandPicker`'s cover-tile shape), not the round wrapping chips this shipped with initially. **No Neon episode/beat model, no `lib/scriptSequenceRunner`, no per-beat pathname/shelf, no last-frame chaining between beats.** The script queue is local panel state. Sequential Render (2026-09-17, explicit ask) fires one paid clip at a time under the existing `runningKind` lock and **stops on first failure** so later lines are not billed — not music-video whole-song auto-render, not a persisted episode. — all explicitly out of scope for this pilot slice per `lib/sunnyBanks.ts`'s and the route's own module doc comments; each is real, working infrastructure elsewhere in this app (`lib/clipRenderBlob.ts`, `lib/scriptSequenceRunner.ts`, `lib/serverVideoFrame.ts`) that Sunny Banks will reuse once there's a real episode model to hang it off, not something to build speculatively now. **Honesty note**: not live-verified in this sandbox (no `ELEVENLABS_API_KEY`/`COMFY_CLOUD_API_KEY` here) — same caveat as every other real-but-unverified backend in this file. | `components/SkidmarksSunnyBanksPanel.tsx`, `lib/sunnyBanks.ts`, `lib/sunnyBanksDropBears.ts`, `lib/sunnyBanksEpisodeBundle.ts`, `lib/sunnyBanksComposite.ts`, `lib/silentMp3.ts`, `app/api/skidmarks/sunnybank/generate-speak-beat/route.ts`, `lib/elevenLabsSpeech.ts` |
...
**The green "Saved ✓" banner has its own small dismiss X now**
(2026-09-17, Stuart's own ask) — purely a local UI dismiss
(`SkidmarksDetailSheet`'s `dismissedSavedBannerFor`), never touches
`sessionSync` itself or the real save it's reporting. Keyed to the
exact `sessionSync.lastSavedAt` it was shown for, so dismissing one
"Saved ✓" can't also hide a genuinely new save that lands later — the
banner reappears the moment `lastSavedAt` moves on. The `"loading"`/
`"saving"`/`"unconfigured"`/`"error"` banners below it have no dismiss
control — only the happy-path "safe to close" one needed clearing.
...
- **"+"** appends one more plate slot to *the same clip's* horizontal
  ...
  second timeline row per beat. Capped at `MAX_PLATES_PER_CLIP` (12,
  raised from 6 on 2026-09-14 — a clip's real render length is segment
  length ÷ plate count, and more plates per clip is how Stuart keeps
  each individual Vocal/LTX render down near ~15s instead of pushing
  toward LTX's 30s ceiling, where its identity tends to drift).
