...
- **Next**: **Sunnybank** — an Australian outback adult-cartoon sitcom,
  ...
  (`lib/sunnyBanks.ts`'s `SUNNY_BANKS_CAST`) plus a "Try one line" form
  that renders one real speak beat (ElevenLabs voice clone → Comfy
  Cloud LTX 2.3 IA2V, `/api/skidmarks/sunnybank/generate-speak-beat`,
  proven in PR #95) **or a Silent Hold** (2026-09-17 — same route,
  `kind: "hold"`, skips ElevenLabs, 5s silent MP3 +
  `buildSunnyBanksHoldPrompt`, still one clip / one tap / still a paid
  LTX render). This is deliberately still just that one-beat pilot
  slice, not the full episode wizard (no episode/beat model, no script
  parser, no per-beat shelf/persistence, no batch render) — see this
  route's and `lib/sunnyBanks.ts`'s own module doc comments before
  extending it. **This bullet was stale until 2026-09-17** (it used to
...
| Sunny Banks (Sunnybank landing tile) | **Real, but deliberately a one-beat pilot, not the episode wizard** (2026-09-15) — `enabled: true` now (was `false`); tapping the tile renders `SkidmarksSunnyBanksPanel`, a **cast strip** of the six locked series regulars (`lib/sunnyBanks.ts`'s `SUNNY_BANKS_CAST` — Shazza, Dazza, Nan, Nuggets, Ranger Bazza, Unit 4S; Hans is a one-episode guest, kept in the same table but never shown in this always-on strip) plus a **"Try one line" form**: pick a character with a real reference plate, type a line, and render one real speak-beat clip via `POST /api/skidmarks/sunnybank/generate-speak-beat` — ElevenLabs text-to-speech (`lib/elevenLabsSpeech.ts`, first caller going TTS-direction rather than Scribe transcription, same `ELEVENLABS_API_KEY`) feeds the same Comfy Cloud LTX 2.3 IA2V graph Skidmarks' own music-video Vocal render already calls (`lib/comfyCloud.ts`, `COMFY_CLOUD_API_KEY`). **Silent Hold (2026-09-17)** is a second one-clip tap on that same form / same route (`kind: "hold"`): skips ElevenLabs, encodes a 5s silent MP3 (`lib/silentMp3.ts`, `SUNNY_BANKS_HOLD_DURATION_SEC`) because LTX's `LoadAudio` node still requires audio, uses `buildSunnyBanksHoldPrompt` (gold, no dialogue, no invented shoes/shadow-silhouette), still a paid Comfy Cloud LTX render — never a batch, never a ShotBlock schema. **Hero still, not the turnaround sheet (2026-09-17)** — live QA: Hold on Shazza animated every cell of `shazza-reference.jpg` because that file is a 4-body + 4-head character plate and the gold prompt keeps the start image's people. Speak/Hold `startImageDataUrl` for LTX is the **composed** plate (location + hero via generate-still); the cast strip still uses `resolveSunnyBanksStartImage` (`*-hero.jpg` cropped once from the sheet; Unit 4S already a single figure). Not a runtime canvas cropper, not a pose picker. **Location plates (2026-09-17)** — six locked full-frame park stills (`SUNNY_BANKS_LOCATIONS`: water tank dam, main entrance, site laundry, office storefront, tin shed & mower, fibro caravan interior). A native Locations `<select>` under the character row. Speak/Hold do **not** send the empty park plate to LTX (live QA: invented sketch woman). Same procedure as original Skidmarks Studio `plateCastIntoGen`: xAI still with Image 1 = location, Image 2 = hero card (`buildSunnyBanksCompositePlatePrompt` + existing `/api/skidmarks/generate-still`), then that composed still is `startImageDataUrl` for LTX. Gold Hold/Speak strings unchanged. `locationId`/`locationImage` ride along unused. Not a sequencer. Every look/voice/style lock string in `lib/sunnyBanks.ts` is copied verbatim from the original Skidmarks Studio repo's own "worked 100%, do not rebuild these" gold doc — never reworded. **Cast thumbnails are square tiles in one horizontally-scrollable strip** (2026-09-17, matches `SkidmarksBandPicker`'s cover-tile shape), not the round wrapping chips this shipped with initially. **No episode/beat model, no script parser, no per-beat pathname/shelf, no last-frame chaining between beats, and never a batch render across a whole episode** — all explicitly out of scope for this pilot slice per `lib/sunnyBanks.ts`'s and the route's own module doc comments; each is real, working infrastructure elsewhere in this app (`lib/clipRenderBlob.ts`, `lib/scriptSequenceRunner.ts`, `lib/serverVideoFrame.ts`) that Sunny Banks will reuse once there's a real episode model to hang it off, not something to build speculatively now. **Honesty note**: not live-verified in this sandbox (no `ELEVENLABS_API_KEY`/`COMFY_CLOUD_API_KEY` here) — same caveat as every other real-but-unverified backend in this file. | `components/SkidmarksSunnyBanksPanel.tsx`, `lib/sunnyBanks.ts`, `lib/silentMp3.ts`, `app/api/skidmarks/sunnybank/generate-speak-beat/route.ts`, `lib/elevenLabsSpeech.ts` |
