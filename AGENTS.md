...
| Auto-plate from a short brief | **Real** — fills *empty* plate slots across the whole clip list with real xAI-generated stills off a small heuristic planner (never an LLM call against the brief), **then stops**; never overwrites a filled plate, never renders video. Its one scripted exception (the door → keyhole → Jack opener for *Talking to Concrete*'s 0:00–0:40) carries Stuart's own exact wording, recreated 2026-09-13 after the Neon-migration data-loss incident — see `CONCRETE_OPENER_SHOTS`'s own inline comments before touching that wording again. **Seedance via SIRAY, and a real "17 positions" system, are the next planned upgrade to this planner** (per-character master-still reference → real generated angle variety instead of the current small template set) — not built yet, `SIRAY_API_KEY` not read anywhere in this repo yet. | `lib/autoPlate.ts`, `components/SkidmarksAutoPlate.tsx` |
